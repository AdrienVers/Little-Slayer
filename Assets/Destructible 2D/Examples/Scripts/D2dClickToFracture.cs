...
namespace Destructible2D
{
	...
	public class D2dClickToFracture_Editor : D2dEditor<D2dClickToFracture>
	{
		protected override void OnInspector()
		{
			...
			DrawDefault("RequireFracturer");

			if (Any(t => t.RequireFracturer == false))
			{
				DrawDefault("PointsPerSolidPixel");
				DrawDefault("MaxPoints");
				DrawDefault("FactorInSharpness");
				DrawDefault("SplitAfterFracture");

				if (Any(t => t.SplitAfterFracture == true))
				{
					BeginIndent();
						DrawDefault("SplitFeather");
						DrawDefault("SplitHealThreshold");
					EndIndent();
				}
			}
		}
